      ...
      {report ? (
        ...
        >
          ...
          <section className="report-section" aria-labelledby="report-milestone-title">
            ...
            {report.milestoneExceptions.length === 0 ? <p>No milestone exception is recorded.</p> : <ul className="report-record-list">{report.milestoneExceptions.map((milestone) => <li key={milestone.id}><AlertTriangle size={18} aria-hidden="true" /><div><strong>{milestone.name} · {milestone.varianceDays > 0 ? "+" : ""}{milestone.varianceDays} days · {milestone.status}</strong><span>{milestone.owner} · baseline {formatDate(milestone.baselineDate)} · previous {formatDate(milestone.previousForecastDate)} · current {formatDate(milestone.forecastDate)}{milestone.actualDate ? ` · actual ${formatDate(milestone.actualDate)}` : ""}</span><p><b>Forecast movement:</b> {milestone.movementDays > 0 ? "+" : ""}{milestone.movementDays} days · dependency evidence {milestone.dependencyQuality}{milestone.dependencyIssues.length > 0 ? ` (${milestone.dependencyIssues.join(", ")})` : ""}</p><p><b>Cause:</b> {milestone.cause ?? "Not recorded"}</p><p><b>Recovery:</b> {milestone.recoveryAction ?? "Not recorded"}</p><p><b>Owner / due:</b> {milestone.actionOwner ?? "Not recorded"} · {milestone.actionDueDate ? formatDate(milestone.actionDueDate) : "No due date"}</p><p><b>Decision:</b> {milestone.decisionRequired ?? "Not recorded"}</p><p>{milestone.commentary}</p></div></li>)}</ul>}
          </section>
